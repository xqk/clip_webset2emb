...
class ClipMapper:
    ...
    def __init__(
        ...
        if clip_model.startswith("open_clip:"):
            model, _ = load_open_clip(
                clip_model, use_jit=use_jit, device=self.device, clip_cache_path=clip_cache_path
            )

        elif clip_model.startswith("cn_clip:"):
            model, _ = load_cn_clip(
                clip_model=clip_model, device=self.device, download_root=clip_cache_path
            )
        else:
            model, _ = load_clip(
